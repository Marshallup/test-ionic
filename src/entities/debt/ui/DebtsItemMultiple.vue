...
import { ListItem, type DebtsByType } from "@/shared";
...
const totalTook = computed(() =>
  props.debts.took.reduce((prev, cur) => prev + cur.count, 0)
);
const totalBorrowed = computed(() =>
  props.debts.borrowed.reduce((prev, cur) => prev + cur.count, 0)
);
